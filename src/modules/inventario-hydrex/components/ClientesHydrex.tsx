...
export function ClientesHydrex({ clientes, onRefresh }: Props) {
  ...
  async function save(e: React.FormEvent) {
    ...
    await upsertCliente({
      ...edit,
      contacto: typeof edit.contacto === 'object' ? edit.contacto : {},
    })
    setEdit(null)
    onRefresh()
  }
  ...
  return (
    <div className="space-y-6">
      <button type="button" onClick={() => setEdit({ nombre: '', contacto: {}, notas: '' })} className="rounded-md bg-zinc-900 px-4 py-2 text-sm text-white">+ Cliente</button>

      <div className="grid gap-6 lg:grid-cols-2">
        <ul className="rounded-lg border divide-y text-sm">
          {clientes.map((c) => (
            <li key={c.id} className="px-4 py-2 flex justify-between items-center">
              <button type="button" className="text-left hover:underline" onClick={() => setSelectedId(c.id)}>
                {c.nombre}
              </button>
              <RowActions
                onEdit={() => setEdit(c as unknown as Record<string, unknown>)}
