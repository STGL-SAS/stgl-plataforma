...
- **Autenticación: Supabase Auth**, sin duda. Es la pieza que maneja el login (correo/contraseña o magic link) y la sesión activa, y le da a cada usuario un ID único. Los roles en sí **no viven dentro de Auth** (Supabase Auth no trae eso de fábrica) — se maneja con una tabla propia (ej. `usuarios_roles`) que conecta ese ID de usuario con su rol, y esa tabla es la que consultan las políticas de RLS para decidir qué puede ver o hacer cada quien.
